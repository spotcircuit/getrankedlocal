...
export default function UnifiedDetailsPage() {
  ...
  const decoded = useMemo(() => {
    ...
    const normalizeNiche = (n: string) => {
      ...
      if (t === 'hairsalons' || t === 'hair-salons') return 'hair salons';
      return n;
    };
    ...
    const fetchBusinessData = async () => {
      try {
        ...
        if (!analyzeRes.ok) {
          ...
          throw new Error(errorData.details || errorData.error || 'Failed to fetch business data');
        }
        ...
        const formattedResults = {
          business: {
            ...
            rating: parseFloat(data.business.rating) || 0,
            review_count: parseInt(data.business.reviewCount) || 0,
          ...
          ai_intelligence: data.business.ai_intelligence || data.additional_data,
          market_analysis: {
            total_competitors: data.analysis?.competitors?.length || 0,
            ...
            market_share_position: 
              (data.analysis?.currentRank || 999) <= 3 ? 'Dominant' : 
              (data.analysis?.currentRank || 999) <= 10 ? 'Competitive' : 
              'Needs Improvement'
          },
          analysis: {
            currentRank: data.analysis?.currentRank,
            potentialTraffic: data.analysis?.potentialTraffic,
            lostRevenue: data.analysis?.lostRevenue,
            reviewDeficit: data.analysis?.reviewDeficit,
            competitors: (data.analysis?.competitors || []).map((c: any) => ({
              name: c.name || c.business_name,
              rank: c.rank || 999,
              reviews: c.reviews || c.review_count || 0,
              rating: c.rating || 0,
              city: c.city,
              website: c.website,
              advantages: [
                ((c.reviews || c.review_count || 0) > (data.business.reviewCount || 0)) ? 
                  `${(c.reviews || c.review_count || 0) - (data.business.reviewCount || 0)} more reviews` : null,
                ((c.rating || 0) > (parseFloat(data.business.rating) || 0)) ? 
                  `Higher rating (${c.rating})` : null
              ].filter(Boolean)
            }))
          }
        };
        ...
        <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Looking for: {decoded.company} in {decoded.city}, {decoded.state}
            </p>
